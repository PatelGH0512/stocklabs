...
export default function HoldingsSection({ onHoldingsChange }: { onHoldingsChange?: (symbols: string[]) => void }) {
  ...
  useEffect(() => {
    // Mock initial holdings for display
    const mock: HoldingItem[] = [
      { id: "h1", symbol: "AAPL", company: "Apple Inc", shares: 1, buyPrice: 233.0, currentPrice: 245.27 },
      { id: "h2", symbol: "MCD", company: "MCDONALD'S CORP", shares: 15, buyPrice: 34.0, currentPrice: 297.01 },
    ];
    setHoldings(mock);
  }, []);
  ...
  useEffect(() => {
    const t = setTimeout(() => searchHoldingStocksDebounced(holdingSearchQuery), 300);
  ...
  const handleConfirmAddHolding = () => {
    ...
    const newHolding: HoldingItem = {
      ...
      currentPrice: Math.max(1, Math.round(Math.random() * 1200 * 100) / 100),
    };
  ...
  return (
    <div className="rounded-xl overflow-hidden border bg-card text-card-foreground mb-6">
      <div className="flex items-center justify-between p-4 border-b border-border">
        ...
        <Button onClick={() => setShowAddHolding(true)} className="bg-primary text-primary-foreground hover:brightness-110 px-6">Add Stocks</Button>
      </div>
                ...
                <tr key={h.id} className="border-t border-border">
                  <td className="p-4">
                    <div className="flex flex-col">
                      <span className="text-foreground font-medium">{h.company}</span>
                      <span className="text-xs text-muted-foreground">{h.symbol}</span>
                    </div>
                  </td>
                  <td className="p-4 text-foreground">${h.currentPrice.toFixed(2)}</td>
                  <td className="p-4 text-foreground">
                    {h.shares} {h.shares === 1 ? "share" : "shares"}
                    <span className="text-xs text-muted-foreground ml-1">@ ${h.buyPrice.toFixed(2)}</span>
                  </td>
                  <td className="p-4 text-foreground">${totalValue.toFixed(2)}</td>
                  <td className="p-4">
                    <div className="flex items-center gap-2">
                      <span className={cn("text-sm font-medium", positive ? "text-green-600" : "text-red-500")}>{positive ? "+" : ""}{pct.toFixed(2)}%</span>
                      <div className="h-2 w-28 rounded-full bg-muted overflow-hidden">
                        <div className={cn("h-2", positive ? "bg-green-200" : "bg-red-200")} style={{ width: `${Math.min(Math.abs(pct), 100)}%` }} />
                      </div>
                    </div>
                    <div className={cn("text-xs", positive ? "text-green-600" : "text-red-500")}>{positive ? "+" : ""}${Math.abs(pnl).toFixed(2)}</div>
                  </td>
                  <td className="p-4 text-right">
                    <div className="flex justify-end items-center gap-2">
                      <button className="text-muted-foreground hover:text-foreground transition-colors" aria-label="Edit">
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                      </button>
                      <button onClick={() => handleDeleteHolding(h.id)} className="text-muted-foreground hover:text-red-500 transition-colors" aria-label="Delete">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="rounded-xl p-6 w-full max-w-md border bg-card text-card-foreground">
            ...
            <div className="space-y-4 mt-2">
              <div>
                <label className="text-muted-foreground text-sm">Search Stock</label>
                <Input
                  ...
                  {holdingSearchResults.map((s) => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-foreground text-sm font-medium">{s.symbol}</p>
              ...
              <div>
                ...
                <Input type="number" min={0} value={holdingShares} onChange={(e) => setHoldingShares(parseFloat(e.target.value) || 0)} className="mt-1 bg-background border-border" />
              </div>

              <div>
                <label className="text-muted-foreground text-sm">Buy Price</label>
                <Input type="number" min={0} value={holdingBuyPrice} onChange={(e) => setHoldingBuyPrice(parseFloat(e.target.value) || 0)} className="mt-1 bg-background border-border" />
              </div>
            </div>
            ...
            <div className="flex justify-end gap-3 mt-6">
              ...
              <Button className="bg-primary text-primary-foreground hover:brightness-110" onClick={handleConfirmAddHolding} disabled={!holdingSelectedStock || holdingShares <= 0}>
                Add Stock
              </Button>
            </div>
          </div>
